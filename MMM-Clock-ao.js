...
Module.register("MMM-Clock-ao", {
  ...
  start() {
    ...
    Log.info(`[MMM-Clock-ao] Start mode=${this._isDev ? "DEV(watch)" : "PROD(build)"}`);
    ...
    if (this._isDev) {
      ...
      Log.info(`[MMM-Clock-ao] Dev poll started (interval=${interval}ms)`);
    }
  },
  ...
  _injectWatch(wrapper, rootId) {
    if (document.querySelector(`script[data-mmm-clock-ao-entry="${rootId}"]`)) {
      Log.info("[MMM-Clock-ao] Watch script already injected");
      return;
    }
    const script = document.createElement("script");
    script.type = "module";
    script.src = this.file("dist/index.js");
    script.dataset.mmmClockAoEntry = rootId;
    script.onerror = () =>
      Log.error(
        "[MMM-Clock-ao] Failed to load dist/index.js (watch). Run 'yarn dev' (watch build) first.",
      );
    wrapper.appendChild(script);
    Log.info("[MMM-Clock-ao] Watch script injected (dist/index.js)");
    // Optional simple polling to detect rebuild and force reload if hash changes (disabled by default)
  },

  _injectProd(wrapper, rootId) {
    if (document.querySelector(`script[data-mmm-clock-ao-entry="${rootId}"]`)) {
      Log.info("[MMM-Clock-ao] Prod script already injected");
      return;
    }
    const script = document.createElement("script");
    script.type = "module";
    ...
    script.dataset.mmmClockAoEntry = rootId;
    script.onerror = () =>
      Log.error("[MMM-Clock-ao] Failed to load dist/index.js. Run 'yarn build'.");
    wrapper.appendChild(script);
    Log.info("[MMM-Clock-ao] Prod script injected (dist/index.js)");
  },

  getDom() {
    const rootId = `${this.identifier}_react_root`;
    const wrapper = document.createElement("div");
    wrapper.className = "mmm-clock-ao-wrapper";

    const root = document.createElement("div");
    root.id = rootId;
    root.className = "mmm-clock-ao-root";
    ...
    try {
      ...
      root.dataset.config = "{}";
      Log.error("[MMM-Clock-ao] Failed to stringify config", e);
    }
    // Placeholder to visualize before React mounts
    root.innerHTML =
      '<div style="color:#888;font-size:12px;">[MMM-Clock-ao] Mounting...</div>';
    ...
    fetch(url, { cache: "no-store" })
      ...
      .then((txt) => {
        ...
        if (this._lastBundleSig && this._lastBundleSig !== sig) {
          Log.info("[MMM-Clock-ao] Detected dist/index.js change -> reloading window");
      ...
      .catch((err) => {
        Log.error("[MMM-Clock-ao] Dev poll fetch failed", err);
      });
  },
});
