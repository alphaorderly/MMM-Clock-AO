...
export function CalendarDisplay({ now }: { now: Date }) {
  ...
  return (
    <div className="flex flex-col items-center w-full px-2  select-none">
      <div className="text-lg font-medium tracking-wider opacity-60 mb-3">
        {year}년 {month}월
      </div>

      <div className="grid grid-cols-7 w-full mb-1 pb-1 border-b border-white/10">
        {DAY_LABELS.map((d, i) => (
          <div
            key={d}
            className={[
              "text-center text-[15px] font-medium py-1 uppercase tracking-wide",
              i === 0 ? "text-red-400/80" : i === 6 ? "text-blue-400/80" : "opacity-40",
            ].join(" ")}
          >
            {d}
          </div>
      ...
      <div className="grid grid-cols-7 w-full gap-0.5">
        {cells.map((day, idx) => {
          ...
          let textColor = "opacity-70";
          if (colIdx === 0) textColor = "text-red-400/80";
          else if (colIdx === 6) textColor = "text-blue-400/80";
